...
fn turn_on1(b: i32) -> i32 {
    1
}
#[allow(unused_variables)]
fn turn_off1(b: i32) -> i32 {
    0
}
fn toggle1(b: i32) -> i32 {
    if b > 0 {
        0
    } else {
        1
...
fn turn_on2(b: i32) -> i32 {
    b + 1
}
fn turn_off2(b: i32) -> i32 {
    if b > 1 {
        b - 1
    } else {
        0
...
fn toggle2(b: i32) -> i32 {
    b + 2
}

impl Action {
    pub fn parse_string(s: &str) -> Action {
        let re = Regex::new("(turn on|turn off|toggle) (\\d+),(\\d+) through (\\d+),(\\d+)")
                     .unwrap();
...
fn main() {
    ...
    println!("Number of bulbs lit: {}, total brightness: {}",
             count1,
             count2);
}
